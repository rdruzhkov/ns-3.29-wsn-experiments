...
#include "aodv-routing-protocol_blackhole.h"
#include "ns3/log.h"
...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::aodv::RoutingProtocol")
    ...
    .AddAttribute ("IsBlackHole",
                   "Is the node a black hole",
                   BooleanValue(false),
                   MakeBooleanAccessor (&RoutingProtocol::SetBlackHoleEnable,
                                        &RoutingProtocol::GetBlackHoleEnable),
                   MakeBooleanChecker())
  ;
  return tid;
}
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  if (IsBlackHole || m_routingTable.LookupRoute (dst, toDst))
    {
      /*
       ...
       */
      if (IsBlackHole || ((rreqHeader.GetUnknownSeqno () || (int32_t (toDst.GetSeqNo ()) - int32_t (rreqHeader.GetDstSeqno ()) >= 0))
          && toDst.GetValidSeqNo () ))
        {
          if (IsBlackHole || (!rreqHeader.GetDestinationOnly () && toDst.GetFlag () == VALID))
            {
              m_routingTable.LookupRoute (origin, toOrigin);
