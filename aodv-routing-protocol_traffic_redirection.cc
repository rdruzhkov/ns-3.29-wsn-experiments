...
{
  this->fsm = ExperimentalFiniteStateMachine();
...
{
  NS_LOG_FUNCTION ("RouteOutput called");
  ...
  if (m_routingTable.LookupValidRoute (dst, rt))
    {
      ...
      if (route->GetOutputDevice()->GetNode()->GetId() == 0) {
          if (!this->fsm.IsNextHopLegal(route->GetGateway())) {
            NS_LOG_DEBUG ("[MY_TAG] Rerouted packet to " << route->GetDestination () << " from interface " << route->GetSource ());
            route->SetGateway(Ipv4Address("10.0.0.3"));
            }
        }
